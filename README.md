...
| **Compute** | ESP32-S3 |
| **Storage** | Local microSD with buffered asynchronous recording |
...
The primary compute and acquisition board combines:

- ESP32-S3 compute
- Battery and power management
- Protected EDA acquisition
- 6-axis IMU capture
- microSD storage
...
A keyed, replaceable optical contact board preserves raw red and infrared PPG measurements for downstream analysis.
...
The architecture separates the human-connected `BAT_HUMAN` domain from the laboratory-facing `LAB_ISO` domain.
...
| Schematic parsing / ERC | KiCad parses and ERC-checks both boards against the allowlist | ✅ Checks pass |
...
A successful run currently ends with:
...
6. Cross-check the [pin allocation](docs/pin-allocation.md), [power budget](docs/preliminary-power-budget.md), and [timing/data model](docs/timing-and-data-model.md).
...
| [`diagrams/`](diagrams/) | Architecture and safety-boundary diagrams |
| [`docs/`](docs/) | Architecture, safety, timing, power, validation, and review documentation |
| [`hardware/design-manifest.json`](hardware/design-manifest.json) | Board hierarchy, nets, parts, GPIO allocation, and unresolved gates |
| [`hardware/interfaces.json`](hardware/interfaces.json) | Connector ordering, electrical domains, and EDA attachment policy |
| [`hardware/circle-main/`](hardware/circle-main/) | Main-board KiCad project |
| [`hardware/circle-ppg/`](hardware/circle-ppg/) | Optical-board KiCad project |
| [`hardware/reports/`](hardware/reports/) | ERC evidence, BOMs, rendered schematics, toolchain record, and verification results |
| [`tools/`](tools/) | Deterministic generation and verification tooling |
| [`tests/`](tests/) | Repository and generated-artifact contract tests |
...
## A note on revision naming

Some current repository artifacts reference **Rev A** while newer architecture and hardware artifacts reference **Rev B**.

Until the revision transition is reconciled across the complete package, this README uses **CIRCLE** as the platform-level name rather than presenting either revision as universally authoritative.

The verification output above is preserved as `CIRCLE Rev A review package: VERIFIED` because that is the current release-check output.
